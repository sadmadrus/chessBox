...
Сервис принимает запрос `POST` с набором параметров в виде `x-www-form-urlencoded` и создаёт игру с заданными параметрам:

- `notify` — URL, на который будут отправляться запросы с уведомлениями;
...
- `201 Created` — игра создана, в заголовке `Location` возвращается адрес с данными созданной игры, состоящий из `/` и ID игры;
- `400 Bad Request` — заданы некорректные параметры (невозможная позиция, нулевое/отрицательное время и т.п.).
...
- `notify` — (необязательный параметр) URL, на который будут отправляться запросы с уведомлениями;
...
Сервис принимает запрос `PUT` с набором параметров в виде `x-www-form-urlencoded`, и обрабатывает соответствующий ход:
...
В процессе игры сервис высылает уведомления об изменениях в ходе игры. Уведомления отправляются в виде запросов `PUT` на адреса, заданные как адреса для уведомлений. Адрес может быть задан для уведомлений при любом запросе (в параметре `notify`); ожидается, что время от времени запрос на уведомления будет подтверждаться. Если для данного адреса запрос на уведомления не поступал некоторое время (заданное при запуске сервиса), он может быть удалён из списка уведомляемых о ходе этой игры.

Уведомления содержат информацию в виде `x-www-form-urlencoded`. Следующие поля (параметры) содержатся в любом уведомлении:
